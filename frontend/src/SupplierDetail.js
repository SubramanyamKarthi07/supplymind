...
const SupplierDetail = () => {
  ...
  useEffect(() => {
    fetch('https://disprove-gains-prognosis.ngrok-free.dev/api/supplier-risk', {
      method:'POST',
      headers:{
        ...
        <ul>
          {
            riskData?.top_features
              ? riskData.top_features.map(f => <li key={f}>{f}</li>)
              : supplier.top_features?.map(f => <li key={f}>{f}</li>)
          }
        </ul>
      </div>
      ...
      <div style={{
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={supplier.trend}>
            ...
            <YAxis />
            <Tooltip />
            <Line type="monotone" dataKey="otif" />
          </LineChart>
        </ResponsiveContainer>
      </div>
